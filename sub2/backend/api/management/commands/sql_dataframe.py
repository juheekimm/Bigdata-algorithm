...
class Command(BaseCommand):
    ...
    # 상점id를 받고 거리를 받아 반경 {dis}km안에 있는 유사 음식점 추천
    def recommendation_category_menu_distance(self, store_id, dis):
    ...
    #상점이름과 지역을 넣고 비슷한 상점 추출
    def recommendation_category_region(self, store_name, region):
        print("[*] Loading store data...")
        data =  self.query_MySqlDB("select * from api_store where address like '%%"+region+"%%' and category != ' '")
